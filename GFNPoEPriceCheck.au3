...
Func AutorunAwakened()
	If Not FileExists($sIniFile) Or IniRead($sIniFile, "AwakenedPath", "ExePath", "") = "" Then
		...
		If $iResponse = $IDYES Then
			Local $sFile = FileOpenDialog("Выберите пробужденный PoE Trade.exe", "C:\Users\" & $sИмя пользователя & "\AppData\Local\Programs\Exiled Exchange 2", "Exiled Exchange 2.exe (*.exe)", $FD_FILEMUSTEXIST)
			If @error Then
